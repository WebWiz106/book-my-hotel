...
const MenuPage = ({ isMenuOpen, menuType }) => {
    return (
             ...
             <ul type='none' className='mt-[4rem] ms-[4rem] gap-2 flex flex-col'>
                {menuType==="menu" && <div className='flex flex-col gap-4'>
                   { menuOptions.map((option)=>{
                    return(
                       <Link to={option}><li className=' text-white hover:bg-white hover:text-black w-[2rem] md:w-[26rem] flex justify-center rounded-md p-2 hover:cursor-pointer'>{option}</li></Link> 
                    )
                   })}
                </div> }
